...
int main(int argc, char **argv) {
  ...
  if (read_mesh) {
    ...
    ierr = DMPlexCreateBoxMesh(PETSC_COMM_WORLD, dim, PETSC_FALSE, m_elem, NULL,
  ...
  ierr = SetupDMByDegree(dm, degree, num_comp_u, dim, true,
  ...
  ierr = SetupVertexDMFromDM(dm, dm_Pi, num_comp_u, true,
  ...
  ierr = SetupLibceedByDegree(dm, ceed, degree, dim, q_extra,
                              dim, num_comp_u, g_size, xl_size, bp_options[bp_choice],
  ...
  }
  {
    ...
    // -- Vectors
    PetscInt num_elem;
    CeedElemRestrictionGetNumElements(ceed_data->elem_restr_u, &num_elem);
    ierr = VecCreate(comm, &X_Pi_r_loc); CHKERRQ(ierr);
    ierr = VecSetSizes(X_Pi_r_loc, num_elem*8, PETSC_DECIDE); CHKERRQ(ierr);
    ierr = VecSetType(X_Pi_r_loc, vec_type); CHKERRQ(ierr);

    // -- Jacobian Matrix
    ierr = MatCreateSeqAIJ(comm, 8*num_elem, 8*num_elem, 8, NULL, &mat_S_Pi_r);
    CHKERRQ(ierr);
